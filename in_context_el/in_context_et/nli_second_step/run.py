...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # true
            model_inputs = tokenizer(dat_true, padding=True, truncation=True, return_tensors='pt')
            model_inputs = model_inputs.to(args.device)
            output = model(**model_inputs)[:, -1]

            # false
            model_inputs_false = tokenizer(dat_false, padding=True, truncation=True, return_tensors='pt')
