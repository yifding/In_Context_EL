...
def llm4ed(
    ...
    # obtain the context
    if add_indicator:
        indicator_context = '<sentence>' + ' ' + left_context + ' ' + '<mention>' + \
                  ' ' + mention + ' ' + '</mention>' + ' ' + right_context + ' ' + '</sentence>'
        if add_doc_name:
            indicator_context = '<doc_name>' + ' ' + doc_name + ' ' + '</doc_name>' + ' ' + indicator_context
    else:
        indicator_context = left_context + mention + right_context
        if add_doc_name:
            indicator_context = doc_name + ' ' + indicator_context
    context = left_context + mention + right_context
    if add_doc_name:
        context = doc_name + ' ' + indicator_context
...
add_indicator=True
verify_entity=False
max_rel_entity_candidates=3
max_blink_entity_candidates=6
add_augmented_context=True
add_false_option=True
augment_context_openai_model='gpt-4o'
multi_choice_openai_model='gpt-4o'
