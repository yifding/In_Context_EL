...
def parse_args():
    ...
    parser.add_argument(
        ...
        # required=True,
        default='/nfs/yding4/In_Context_EL/RUN_FILES/4_13_2023/rel_blink/entity_candidate_prompt_palm',
        type=str,
    )
    parser.add_argument(
        "--output_file",
...
def main():
    ...
    for doc_name, instance in tqdm(doc_name2instance.items()):
        ...
        for (
            ...
            tmp_complete_output = palm.generate_text(
                model='models/text-bison-001',
                prompt=multi_choice_prompt,
                temperature=0,
                max_output_tokens=500,
            )
            complete_output = tmp_complete_output.result

            multi_choice_prompts.append(multi_choice_prompt)
            multi_choice_prompt_results.append(complete_output)
            tmp_rate_limit += 1
            if tmp_rate_limit >= 0.8 * rate_limit:
